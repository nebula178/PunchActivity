...
Page({
  ...
  joinIn: function(){ //加入打卡活动
    const that = this
    wx.cloud.database().collection('participant').add({
      data: {
  ...
  //  console.log(e)
    wx.redirectTo({
      url: '/pages/clock/clock?id=' + e.currentTarget.dataset.id,
    })
  },
  ...
  judgeButton: function(){
    const that = this
  //  console.log('进入判断')
    wx.cloud.database().collection('once_total').where({  //是否已经加入活动
      cardid: that.data.cardId,
  ...
  onLoad(options) {
    ...
    if(options.scene){  //扫码
      //卡片信息对象赋值
      wx.cloud.database().collection('control_once').where({  //页面展示
        _id: options.scene
      }).get({
        success(result){
      ...
      if(app.globalData.userInfo != null){  //登录了
        wx.cloud.database().collection('once_total').where({  //是否已经加入活动
          cardid: options.scene,
          _openid: app.globalData.openid
        }).get({
